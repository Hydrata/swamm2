...
RUN mkdir -p /usr/src/swamm2

WORKDIR /usr/src/swamm2
...
RUN chmod +x /usr/src/swamm2/tasks.py \
    && chmod +x /usr/src/swamm2/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

ENTRYPOINT ["/usr/src/swamm2/entrypoint.sh"]